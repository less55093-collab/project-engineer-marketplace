...
## Step 1: 默认走深度需求访谈 | Deep Interview by Default

基于 `$ARGUMENTS`，默认进入**重型需求澄清**，不是轻量确认。

核心原则：

- `init` 默认就是 deep mode，不要假设用户已经想清楚
- 默认可以**直接抛出一批问题**，帮助用户系统性思考
- 如果回答仍模糊，就继续追问，不要为了赶快生成文档而提前收束
- 在 `PRD readiness gates` 没满足之前，不要开始落文档

### 提问策略

默认第一轮就可以按主题直接问一批问题。不要怕问题多，重点是让用户把脑子里的模糊需求摊开。
...
如果用户回答很散，就第二轮继续按缺口追问；如果用户回答已经很完整，再进入结构化回读。
...
### 默认第一轮问题包 | Default First-Round Question Pack

默认第一轮不再只问 3 个问题，而是优先从下面问题里选出**一整组**高价值问题直接问：
...
如果 `$ARGUMENTS` 已经非常详细，可以跳过已知问题，但默认仍然应补问多个关键维度，而不是象征性问 1-2 句。

### 后续追问原则 | Follow-up Pressure Rules

第二轮起，不要求覆盖更多维度，而是优先补最危险的缺口。对模糊回答优先用这些方式继续追问：
