...
- `/pe:init`：默认就会做与本命令相同的深度访谈，然后再生成项目入口文档
- `/pe:deep-interview`：当你只想显式进入相同访谈协议，但以 `PRD.md` 为主输出，或在项目中途重新把需求问透时使用
...
`/pe:init` 现在默认已经把这一整套访谈机制完整并入；本命令只是显式保留同一访谈引擎的 PRD-first 出口。
